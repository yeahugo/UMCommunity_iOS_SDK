...
{
    __weak typeof(self) weakSelf = self;
    if ([obj isKindOfClass:[UMComFeedsTableViewCell class]]) {
        [[UMComAction action] performActionAfterLogin:nil viewController:self completion:^(NSArray *data, NSError *error) {
            UMComFeedDetailViewController *detailVC = [[UMComFeedDetailViewController alloc]initWithFeed:feed];
            detailVC.showType = UMComShowFromClickComment;
