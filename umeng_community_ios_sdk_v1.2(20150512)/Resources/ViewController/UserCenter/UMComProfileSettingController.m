...
//    UIBarButtonItem *backButtonItem = [[UMComBarButtonItem alloc] initWithTitle:UMComLocalizedString(@"Back",@"返回") target:self action:@selector(onClickSave)];
...
- (void)setUserProfile
{
    ...
    if (self.user.icon_url) {
        [self.userPortrait  setImageURL:[self.user.icon_url valueForKey:@"240"] placeHolderImage:nil];
    }
...
{
    if (self.user.icon_url) {
        [self.userPortrait  setImageURL:[self.user.icon_url valueForKey:@"240"] placeHolderImage:[UMComImageView placeHolderImageGender:self.user.gender.integerValue]];
    }
    [self.nameField setText:self.user.name];
