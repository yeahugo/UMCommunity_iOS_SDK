...
- (void)viewDidLoad
{
    ...
    if (self.realTextView.text.length == 0 && !self.originFeed) {
...
- (void)setUpAddedImageView:(NSArray *)images
{
    if(!self.addedImageView)
    {
        ...
        self.addedImageView = [[UMComAddedImageView alloc] initWithUIImages:nil screenWidth:self.editBgView.frame.size.width];
...
- (void)viewsFrameChange
{
    ...
    if (self.originFeed) {
        self.realTextView.frame = CGRectMake(0, 0, self.editBgView.frame.size.width,visibleHeight - self.forwardFeedBackground.frame.size.height-5);
        self.forwardFeedBackground.frame = CGRectMake(self.forwardFeedBackground.frame.origin.x, self.realTextView.frame.size.height+2, self.forwardFeedBackground.frame.size.width,self.forwardFeedBackground.frame.size.height);
...
//
        self.realTextView.frame = CGRectMake(0, 0, self.realTextView.frame.size.width,visibleHeight-forwordViewHeight);
    ...
    if (self.locationLabel.text.length > 0 && [self.addedImageView.arrayImages count] > 0) {
        self.locationBackgroundView.frame = CGRectMake(self.addedImageView.frame.origin.x+self.addedImageView.imageSpace-8, self.locationBackgroundView.frame.origin.y, self.locationBackgroundView.frame.size.width, self.forwardFeedBackground.frame.size.height);
    }
