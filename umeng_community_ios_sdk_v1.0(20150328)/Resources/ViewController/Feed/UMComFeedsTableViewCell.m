...
-(void)reload:(UMComFeed *)feed tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    if ([imagesArray count] == 0) {
        ...
        if (self.feed.origin_feed  && !self.feed.origin_feed.isFault) {
            ...
            self.gridView.center = CGPointMake(self.gridView.center.x, self.gridView.center.y);
        }
...
+ (float)getCellHeightWithFeed:(UMComFeed *)feed isShowComment:(BOOL)isShowComment tableViewWidth:(float)viewWidth
{
    ...
    if (origin_feed) {
        ...
        CGRect originTextRect = [UMComMutiStyleTextView boundingRectWithSize:CGSizeMake(viewWidth-TableViewDeltaWidth, MAXFLOAT) font:UMComFontNotoSansLightWithSafeSize(15) string:originText lineSpace:TextViewLineSpace];
