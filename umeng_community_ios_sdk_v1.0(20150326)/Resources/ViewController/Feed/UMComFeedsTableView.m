...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    [self.feedTableViewController postCommentContent:textField.text feedID:self.commentFeedId
...
-(void)refreshFeedsData
{
    _loadingMore = YES;
    [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:YES];
    [self.feedTableViewController.fetchFeedsController fetchRequestFromServer:^(NSArray *data, BOOL haveNextPage, NSError *error) {
        _loadingMore = NO;
        if (data.count > 0) {
            ((UMComFeedTableViewController *)self.viewController).resultArray = [NSMutableArray arrayWithArray:data];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.heightDictionary.count > 0 && self.heightDictionary.count >= indexPath.row && [self.heightDictionary valueForKey:key]) {
        ...
        UMComFeed *feed = [[self feedTableViewController].resultArray objectAtIndex:indexPath.row];
        BOOL isShowAll = [(UMComFeedsTableView *)self isShowAllComment:(int)indexPath.row];
        cellHeight = [UMComFeedsTableViewCell getCellHeightWithFeed:feed isShowComment:isShowAll];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    else if ([self feedTableViewController].resultArray.count >= kFetchLimit && offset + self.superview.frame.size.height > self.contentSize.height){
...
{
    NSInteger indexStart = [self feedTableViewController].resultArray.count;
    ...
    if (data.count> 0) {
        [self.heightDictionary removeAllObjects];
        [[self feedTableViewController].resultArray addObjectsFromArray:data];
