...
- (void)viewDidLoad
{
    ...
    if (self.originFeed) {
        ...
        self.atFriendButton.center = CGPointMake(self.editToolView.frame.size.width/2,self.editToolView.frame.size.height/2);
        [self.editToolView updateConstraints];
        ...
        if (self.originFeed.images && [self.originFeed.images count] > 0) {
            ...
            [self.forwardFeedBackground updateConstraints];
        }
...
- (void)createTextViewios7
{
    ...
    self.realTextView = [[UITextView alloc] initWithFrame:self.fakeTextView.frame textContainer:container];
    self.realTextView.translatesAutoresizingMaskIntoConstraints = NO;
...
//            [weakSelf viewsFrameChange];
        ...
        [self.addedImageView setOrign:CGPointMake(10,21)];
        
        self.addedImageView.frame = CGRectMake(10, 21, self.forwardFeedBackground.frame.size.width, 80);
...
//  
//    if (self.originFeed) {
//        self.realTextView.frame = CGRectMake(self.fakeTextView.frame.origin.x, self.fakeTextView.frame.origin.y, self.fakeTextView.frame.size.width,visibleHeight - self.forwardFeedBackground.frame.size.height - 3);
//        self.forwardFeedBackground.frame = CGRectMake(self.forwardFeedBackground.frame.origin.x, self.fakeTextView.frame.origin.y+self.fakeTextView.frame.size.height, self.forwardFeedBackground.frame.size.width,self.forwardFeedBackground.frame.size.height);
//    }else{
//        CGFloat forwordViewHeight = 10;
...
//                self.addedImageView.frame = CGRectMake(0, self.locationBackgroundView.frame.size.height, self.view.frame.size.width, self.addedImageView.frame.size.height);
//                self.addedImageView.contentSize = CGSizeMake(self.view.frame.size.width, self.addedImageView.contentSize.height);
//            }else{
...
//            self.locationBackgroundView.hidden = YES;
//        }
//        self.realTextView.frame = CGRectMake(self.fakeTextView.frame.origin.x, self.fakeTextView.frame.origin.y, self.fakeTextView.frame.size.width,visibleHeight-forwordViewHeight);
//        self.forwardFeedBackground.frame = CGRectMake(0, self.fakeTextView.frame.origin.y+self.fakeTextView.frame.size.height, self.view.frame.size.width,forwordViewHeight);
//    }
}
...
//    self.editToolView.frame = CGRectMake(self.editToolView.frame.origin.x,keybordFrame.origin.y - self.editToolView.frame.size.height, self.editToolView.frame.size.width, self.editToolView.frame.size.height);
    
//    self.topicsViewController.view.frame = CGRectMake(0,self.view.frame.size.height-endheight, self.view.frame.size.width, endheight);
...
-(void)keyboardDidShow:(NSNotification*)notification
{
    ...
    float endheight = keybordFrame.size.height;
    self.visibleViewHeight = self.view.frame.size.height - endheight - self.editToolView.frame.size.height;
    self.editToolView.frame = CGRectMake(self.editToolView.frame.origin.x, self.view.bounds.size.height - endheight - self.editToolView.frame.size.height, self.editToolView.frame.size.width, self.editToolView.frame.size.height);
    self.forwardFeedBackground.frame = CGRectMake(10, self.editToolView.frame.origin.y-ForwardViewHeight-5, self.forwardFeedBackground.frame.size.width, ForwardViewHeight);
    self.addedImageView.frame = CGRectMake(0, 21, self.forwardFeedBackground.frame.size.width, 80);
    self.addedImageView.contentSize = CGSizeMake(self.forwardFeedBackground.frame.size.width, self.addedImageView.contentSize.height);
    self.realTextView.frame = CGRectMake(0, 0, self.fakeTextView.frame.size.width, self.forwardFeedBackground.frame.origin.y);
    self.topicsViewController.view.frame = CGRectMake(0,self.view.frame.size.height-endheight, self.view.frame.size.width, endheight);
    [self.topicsViewController.tableView setContentInset:UIEdgeInsetsZero];
//    self.forwardFeedBackground.backgroundColor = [UIColor redColor];
//    UMLog(@"addedImageView:%@",NSStringFromCGRect(self.addedImageView.frame));
//    UMLog(@"forwardFeedBackground:%@",NSStringFromCGRect(self.forwardFeedBackground.frame));
//    UMLog(@"editToolView:%@",NSStringFromCGRect(self.editToolView.frame));
...
- (void)postContent
{
    ...
    if (self.realTextView.text && self.realTextView.text.length > 300) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:UMComLocalizedString(@"Sorry",@"抱歉") message:UMComLocalizedString(@"The content is too long",@"内容过长") delegate:nil cancelButtonTitle:UMComLocalizedString(@"OK",@"好") otherButtonTitles:nil];
        [alertView show];
        [self.realTextView becomeFirstResponder];
