...
/**
 ...
 @param user 被禁言用户
 @param topics 禁言的话题`UMComTopic`对象列表，即管理员可以根据自己的权限对某个用户在某个话题下禁言，全局管理员则具有任何话题下的禁言权限
 @param ban   是否禁言，如果为YES表示禁言，如果为NO表示取消禁言
 @param result 操作结果回调
 */
...
/**
 ...
 @warning 这些数据都会保存在UMComSession单利里面 `config.feed_lenght`对应的UMComSession的maxFeedLength属性，`message_box`保存在UMComSession的unReadNoticeModel属性里面，他是`UMComUnReadNoticeModel`类的对象，
 ...
 */
+ (void)initChartBoxWithToUser:(UMComUser *)user
