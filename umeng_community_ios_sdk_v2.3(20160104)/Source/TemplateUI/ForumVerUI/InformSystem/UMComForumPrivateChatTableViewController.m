...
- (void)viewWillLayoutSubviews
{
    [super viewWillLayoutSubviews];
     self.tableView.frame = _changedFrame;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < self.chatDataArray.count) {
        [cell reloadTabelViewCellWithMessage:nil mutiText:mutiText cellSize:CGSizeMake(tableView.frame.size.width, mutiText.textSize.height+40)];
    }
    return cell;
}
...
- (void)reloadTabelViewCellWithMessage:(UMComPrivateMessage *)privateMessage mutiText:(UMComMutiText *)mutiText cellSize:(CGSize)size
{
    ...
    self.dateLabel.text = [[NSDate date] description];//privateMessage.create_time;
