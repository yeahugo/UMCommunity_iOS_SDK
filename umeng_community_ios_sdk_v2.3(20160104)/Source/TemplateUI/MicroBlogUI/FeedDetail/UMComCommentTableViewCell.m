...
//    CGSize textSize = [self.timeLabel.text sizeWithFont:self.timeLabel.font constrainedToSize:CGSizeMake(self.timeLabel.frame.size.width, self.timeLabel.frame.size.height) lineBreakMode:NSLineBreakByCharWrapping];
    ...
    if (comment.content) {
        ...
        self.commentTextView.frame = CGRectMake(self.commentTextView.frame.origin.x,self.timeLabel.frame.origin.y+self.timeLabel.frame.size.height+5, self.commentTextView.frame.size.width, mutiText.textSize.height-self.commentTextView.frame.origin.y/2);
