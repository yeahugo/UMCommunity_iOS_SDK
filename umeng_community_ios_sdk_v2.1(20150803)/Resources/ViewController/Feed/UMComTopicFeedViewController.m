...
-(id)initWithTopic:(UMComTopic *)topic
{
    ...
    if (self) {
        self.topic = topic;
        UMComTopicFeedsRequest *topicFeedsController = [[UMComTopicFeedsRequest alloc] initWithTopicId:topic.topicID count:TotalTopicSize order:UMComFeedSortTypeDefault isReverse:YES];
...
- (void)viewDidLoad {
    ...
    self.feedsTableView.frame = CGRectMake(0, 0, self.feedsTableView.frame.size.width, self.feedsTableView.frame.size.height);
    self.feedsTableView.headView = [self creatHeadView];
    self.feedsTableView.footView = [self creatFootView];
    ...
    self.hotUsersTableView.headView = [self creatHeadView];
    self.hotUsersTableView.footView = [self creatFootView];
...
{
    UMComRefreshView *headView = [[UMComRefreshView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.followBackground.frame.size.height)];
    return headView;
}

- (UMComRefreshView *)creatFootView
{
...
- (void)onClickHotUserFeedsButton:(id)sender {
    ...
    self.hotUsersTableView.scrollViewDelegate = self;
    if (self.hotUsersTableView.userList.count == 0) {
        [self.hotUsersTableView refreshAllData];
    }
...
- (void)customObj:(id)obj clickOnFollowUser:(UMComUser *)user
{
    ...
    [[UMComAction action] performActionAfterLogin:nil viewController:self completion:^(NSArray *data, NSError *error) {
        if ([weakCell isKindOfClass:[UMComUserTableViewCell class]]) {
            [weakCell focusUserAfterLoginSucceed];
        }
    }];
}
