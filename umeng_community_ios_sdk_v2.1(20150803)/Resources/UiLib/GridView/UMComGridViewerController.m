...
- (UMZoomScrollView *)zoomScrollViewWithPageIndex:(NSInteger)pageIndex imageLocation:(ImageLocation)imageLocation
{
    if (self.arrayUrl.count > 0) {
        ...
        if (self.arrayUrl.count > pageIndex) {
            NSArray *arr = (NSArray *)self.arrayUrl[pageIndex];
            ;
            [zoomView.imageView setImageURL:[NSURL URLWithString:(NSString *)arr[1]]];
            
            [zoomView.imageView setThumImageViewUrl:(NSString *)arr[0]];
        }
        return zoomView;
    }
    return nil;
