...
+ (void)handleUserInfo:(NSDictionary *)userInfo
{
    ...
    if ([userInfo valueForKey:@"umwsq"]) {//[userInfo valueForKey:@"umeng_comm_afteropen_controller"] && [[userInfo valueForKey:@"umeng_comm_afteropen_controller"] isEqualToString:@"UMComFeedDetailViewController"]
        ...
        if ([feed_id isKindOfClass:[NSString class]]) {
            UMComFeedDetailViewController *feedDetailVC = [[UMComFeedDetailViewController alloc]initWithFeed:feed_id commentId:comment_id];
            if ([userInfo valueForKey:@"comment_id"]){
