...
- (void)initData
{
    ...
    self.titlelabel.font = UMComFontNotoSansDemiWithSafeSize(12);
...
- (void)creatNouserTipView
{
    ...
    label.text = UMComLocalizedString(@"no_related_user", @"暂无相关用户");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (indexPath.row < self.userList.count) {
        ...
        cell.userImageView.image = [UIImage imageNamed:@"all_more"];
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        self.userImageView = [[UMImageView alloc]initWithFrame:CGRectMake(0, 0, imageHeight, imageHeight)];
...
- (void)setUser:(UMComUser *)user
{
    ...
    [self.userImageView setIsAutoStart:NO];
    [self.userImageView setImageURL:[NSURL URLWithString:iconUrl]];
    if (!self.userImageView.isCacheImage) {
        UIImage *placeHolderImage = [UIImage imageNamed:@"male"];
        if ([user.gender intValue] == 0) {
            placeHolderImage = [UIImage imageNamed:@"female"];
        }
        [self.userImageView setPlaceholderImage:placeHolderImage];
        [self.userImageView startImageLoad];
    }
}
