...
//    //请求详细信息
    ...
    self.feedsTableView.deletedFeedSucceedAction = ^(){
        if ([weakSelf.user.uid  isEqualToString:[UMComSession sharedInstance].uid]) {
            if ([UMComSession sharedInstance].profile) {
                int feed_count = [[UMComSession sharedInstance].profile.feed_count intValue];
                if (feed_count > 0) {
                    feed_count -- ;
                }
                [UMComSession sharedInstance].profile.feed_count = [NSString stringWithFormat:@"%d",feed_count];
                weakSelf.feedNumber.text = [UMComSession sharedInstance].profile.feed_count;
            }
        }
    };
    ...
    [(UMComUserCenterViewModel *)self.feedViewModel requestFollowUser:self.focus completion:^(NSError *error) {
        if (error) {
            ...
            if (self.feedViewModel.isFocus) {
                self.user.user_profile.fans_count = [NSString stringWithFormat:@"%d",[self.user.user_profile.fans_count intValue] + 1];
                NSInteger followersCount = [[UMComSession sharedInstance].profile.following_count integerValue] +1;
                [UMComSession sharedInstance].profile.following_count = [[NSNumber numberWithInteger:followersCount] stringValue];
            } else {
                self.user.user_profile.fans_count = [NSString stringWithFormat:@"%d",[self.user.user_profile.fans_count intValue] - 1];
                NSInteger followersCount = [[UMComSession sharedInstance].profile.following_count integerValue] -1;
                [UMComSession sharedInstance].profile.following_count = [[NSNumber numberWithInteger:followersCount] stringValue];
            ...
            if (self.feedViewModel.isFocus) {
                [self.user.user_profile setValue:@1 forKey:@"is_follow"];
            } else {
                [self.user.user_profile setValue:@0 forKey:@"is_follow"];
            }
            [self setFocus:self.focus];
            [self.fanNumber setText:self.user.user_profile.fans_count];
        }
    ...
    [(UMComUserCenterViewModel *)self.feedViewModel loadDataWithType:UMComUserCenterDataFollow completion:^(NSArray *data,BOOL haveNextPage, NSError *error) {
        if ([data isKindOfClass:[NSArray class]]) {
            ...
            if (data.count > 0) {
                ...
                if ([self.user.user_profile.following_count integerValue] <= 0) {
                    self.followerNumber.text = [NSString stringWithFormat:@"%d",data.count];
                }
            } else {
                self.tipLabel.center = CGPointMake(self.followerTableView.frame.size.width/2, self.followerTableView.frame.size.height/2);
...
-(IBAction)onClickFans:(id)sender
{
    ...
    [(UMComUserCenterViewModel *)self.feedViewModel loadDataWithType:UMComUserCenterDataFans completion:^(NSArray *data,BOOL haveNextPage, NSError *error) {
        ...
        if (data.count > 0) {
            self.tipLabel.hidden = YES;
            if ([self.user.user_profile.fans_count integerValue] <= 0) {
                self.fanNumber.text = [NSString stringWithFormat:@"%d",data.count];
            }
        } else {
...
- (void)updateUserProfile
{
    ...
    [userProfile fetchRequestFromCoreData:^(NSArray *data, NSError *error) {
        UMComUserProfile *userProfile = data.firstObject;
        if (userProfile) {
    ...
    [(UMComUserCenterViewModel *)self.feedViewModel loadProfile:^(NSArray *data, NSError *error) {
        ...
        @try {
            if (data.count > 0) {
                UMComUserProfile *profile = data.firstObject;
                if (profile) {
...
- (void)refreshBaseInformationWithUserProfile:(UMComUserProfile *)profile
{
    
    [self.feedNumber setText:profile.feed_count];
    [self.followerNumber setText:profile.following_count];
    [self.fanNumber setText:profile.fans_count];    
    BOOL isFollow = [profile.is_follow boolValue];
    if (isFollow) {
    ...
    if (profile.name && profile.name.length > 0) {
        [self.userName setText:profile.name];
    }
    if ([self.uid isEqualToString:[UMComSession sharedInstance].uid]) {
        [UMComSession sharedInstance].profile = profile;
    }
    if (self.user.managedObjectContext == profile.managedObjectContext) {
        self.user.user_profile = profile; 
    }
    UMComGenderView *genderView = (UMComGenderView *)[self.view viewWithTag:10000];
    if ([profile.gender integerValue] == 1) {
