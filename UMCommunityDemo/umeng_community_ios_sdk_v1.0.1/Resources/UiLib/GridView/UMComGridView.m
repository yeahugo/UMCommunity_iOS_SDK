...
- (id)initWithArray:(NSArray *)array placeholder:(UIImage *)placeholder cellPad:(NSUInteger)cellPad
{
    ...
    if(self)
    {
        ...
        for(int i = 0; i<9; i++)
        {
            UMImageView *iv = [[UMImageView alloc] initWithPlaceholderImage:placeholder];
...
//            [iv setFrame:getImageViewRect(i, self. cellPad)];
...
//    [self removeAllSubviews];
    ...
    if([array count])
    {
        ...
        int curImageCount = 0;
//        self.imageDeltaWidth = (self.frame.size.width - 3 * IMAGE_CELL_WIDTH)/3;
        ...
        for (int i = 0;i < array.count;i++)
        {
            ...
            if([(NSArray *)arr count]==2)
            {
                if (curImageCount < self.arrayImageView.count) {
                    UMImageView *iv = self.arrayImageView[curImageCount];
                    iv.needCutOff = YES;
                    if (!iv.superview) {
                        [self addSubview:iv];
                    }
                    
                    [iv setPlaceholderImage:self.placeholder];
                   
                    [iv setImageURL:[NSURL URLWithString:(NSString *)((NSArray *)arr[0])]];
        ...
        for (NSInteger i = array.count; i < 9; i++) {
            UMImageView *iv = self.arrayImageView[i];
...
- (void)tapImageView:(UITapGestureRecognizer *)tapGesture
{
    ...
    {
        [self.viewerController setArray:self.arrayUrl index:index];
    }
