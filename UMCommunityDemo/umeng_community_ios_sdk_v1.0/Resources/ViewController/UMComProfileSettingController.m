...
-(void)viewDidLoad
{
    ...
    int gender = [[UMComSession sharedInstance].profile.gender intValue];
        ...
        [userProfileController fetchRequestFromServer:^(NSArray *data, BOOL haveNextPage, NSError *error) {
            if (!error) {
                if (data && data.count > 0) {
                    [UMComSession sharedInstance].profile = data.firstObject;
    ...
    self.topLineView.frame = CGRectMake(0, self.topLineView.frame.origin.y, self.view.frame.size.width, 0.5);
    
    self.mindleLineView.backgroundColor = [UMComTools colorWithHexString:TableViewSeparatorColor];
    self.mindleLineView.frame = CGRectMake(0, self.mindleLineView.frame.origin.y, self.view.frame.size.width, 0.5);
    
    self.bottomLineView.backgroundColor = [UMComTools colorWithHexString:TableViewSeparatorColor];
    self.bottomLineView.frame = CGRectMake(0, self.bottomLineView.frame.origin.y, self.view.frame.size.width, 0.5);
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if (selectImage) {
        ...
        [UMComUpdateProfileImageRequest updateWithProfileImage:selectImage completion:^(NSError *error) {
...
- (void)setUserProfile
{
    ...
    //fix #324 by djx
    if ([UMComSession sharedInstance].profile &&[[UMComSession sharedInstance].profile.gender intValue] == 0) {
        [self.genderButton setTitle:UMComLocalizedString(@"Female",@"女") forState:UIControlStateNormal];
    } else {
        [self.genderButton setTitle:UMComLocalizedString(@"Male",@"男") forState:UIControlStateNormal];
    }
...
- (void)setUserProfile:(UMComUserProfile *)profile
{
    ...
    [self.nameField setText:self.user.name];
    if ([UMComSession sharedInstance].profile && [[UMComSession sharedInstance].profile.gender integerValue] == 0) {
        [self.genderButton setTitle:UMComLocalizedString(@"Female",@"女") forState:UIControlStateNormal];
    } else {
        [self.genderButton setTitle:UMComLocalizedString(@"Male",@"男") forState:UIControlStateNormal];
    }
...
-(void)onClickSave
{
    ...
    if (self.registerError) {
        ...
        [UMComUpdateProfileRequest updateWithUser:userAccount completion:^(NSError *error) {
            if (!error) {
                [[NSNotificationCenter defaultCenter] postNotificationName:UpdateUserProfileSuccess object:self];
