...
- (void)feedDeletedFinishAction:(NSNotification *)notification
{
    if ([notification.object isKindOfClass:[UMComFeed class]]) {
        ...
        [self.dataArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if ([feed.feedID isEqualToString:feedStyle.feed.feedID]) {                [weakSelf.dataArray removeObjectAtIndex:idx];
                [weakSelf reloadData];
                *stop = YES;
            }
        }];
...
- (void)insertFeedStyleToDataArrayWithFeed:(UMComFeed *)feed
{
    ...
    if ([feed isKindOfClass:[UMComFeed class]]) {
        UMComFeedStyle *newFeedStyle = [UMComFeedStyle feedStyleWithFeed:feed viewWidth:self.frame.size.width feedType:feedDefaultType];
        [self.dataArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
