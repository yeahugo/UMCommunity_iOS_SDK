...
{
    if (indexPath.section == 0) {
        return self.feedStyle.totalHeight + DeltaHeight + 5 + self.likeListView.frame.size.height;
    }else{
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        if (operationType == FeedType) {
            ...
            UMComComment  *comment = self.selectedComment;
            if ([self.actionTableView.selectedTitle isEqualToString:@"删除"]) {
...
- (void)deleteCommentWithCommentId:(NSString *)commentId
{
    [UMComCommentFeedRequest postDeleteWithComment:commentId feedId:self.feed.feedID completion:^(id responseObject, NSError *error) {
        if (!error) {
            ...
            UMComUser *user = [UMComSession sharedInstance].loginUser;
            if (error.code == ERR_CODE_USER_NO_PRIVILEGE && [user.permissions containsObject:Permission_delete_content]) {
                [user.permissions removeObject:Permission_delete_content];
                [[UMComCoreData sharedInstance] saveManagedObject:user];
            }
        }
    }];
}
...
- (void)customObj:(id)obj clickOnLikeFeed:(UMComFeed *)feed
{
    ...
    [[UMComAction action] performActionAfterLogin:feed viewController:self completion:^(NSArray *data, NSError *error) {
        if ([feed.liked boolValue] == YES) {
            [UMComLikeFeedRequest postDisLikeWithFeedId:feed.feedID completion:^(NSError *error) {
                if (!error) {
                    feed.liked = @(0);
            ...
            [UMComLikeFeedRequest postLikeWithFeedId:feed.feedID completion:^(id responseObject, NSError *error) {
                if (!error) {
                    feed.liked = @(1);
                    feed.likes_count = [NSNumber numberWithInteger:[feed.likes_count integerValue] +1];
                    [weakSelf refreshFeedsLike:feed.feedID block:nil];
...
{
    __weak typeof(self) weakSelf = self;
    [UMComCommentFeedRequest postWithSourceFeedId:self.feed.feedID commentContent:content replyUserId:self.replyUserId completion:^(NSError *error) {
        if (error) {
            [UMComShowToast showFetchResultTipWithError:error];
        }else{
