...
{
    [self.userPortrait  setImageURL:[self.user.icon_url valueForKey:@"240"] placeHolderImage:[UMComImageView placeHolderImageGender:self.user.gender.integerValue]];    [self.nameField setText:self.user.name];
    ...
    if (self.registerError) {
        ...
        [UMComHttpManager updateProfile:userAccount response:^(id responseObject, NSError *error) {
            if (!error) {
                ...
                weakSelf.user.name = userAccount.name;
                weakSelf.user.gender = userAccount.gender;
            } else {
                [UMComShowToast showFetchResultTipWithError:error];
