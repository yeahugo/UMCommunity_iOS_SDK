...
- (void)initViews
{
    _textCountMarker.text = [NSString stringWithFormat:@"0/%ud", _maxLength];
...
- (IBAction)commit:(id)sender
{
    ...
    if ([UMComTools getStringLengthWithString:_textView.text] > _maxLength) {
        [[UMComiToast makeText:[NSString stringWithFormat:@"回复长度最多为%ud", _maxLength]] show];
        return;
    }
    if (_commitBlock) {
...
{
    _textCountMarker.text = [NSString stringWithFormat:@"%ud/%ud", [UMComTools getStringLengthWithString:_textView.text], _maxLength];
