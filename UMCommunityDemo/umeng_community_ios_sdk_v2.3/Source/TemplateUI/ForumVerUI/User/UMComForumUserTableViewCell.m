...
@interface UMComForumUserTableViewCell ()

@property (nonatomic, strong) UMComUser *user;

@end

@implementation UMComForumUserTableViewCell
                     ...
                     cellSize:(CGSize)size
{
    ...
    if (self) {
        ...
        nameFrame.size.width = size.width-iconWidth-iconLeftEdge*2 - focuseButtonWith - UMCom_Forum_User_Icon_RightEdge;
...
- (void)reloadCellWithUser:(UMComUser *)user
{
    ...
    self.feedCountLabel.text = [NSString stringWithFormat:@"发消息数：%@",user.feed_count];
    self.fansCountLabel.text = [NSString stringWithFormat:@"粉丝：%@",user.fans_count];
