...
- (void)viewDidLoad {
    ...
    if (self.fetchRequest && self.isAutoStartLoadData ) {
    ...
    _indicatorView.center = CGPointMake(self.view.frame.size.width/2,self.noDataTipLabel.frame.origin.y + 40);
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (self.dataArray.count > 0) {
        ...
        if (self.dataArray.count >= self.fetchRequest.fetchRequest.fetchLimit) {
    ...
    switch (loadStatus) {
        ...
        case UMComFinish:
        {
            ...
            if (isPull) {
                ...
                self.statusLable.text = UMComLocalizedString(@"Please login", @"请登录，查看下一页");
            }
            else{
