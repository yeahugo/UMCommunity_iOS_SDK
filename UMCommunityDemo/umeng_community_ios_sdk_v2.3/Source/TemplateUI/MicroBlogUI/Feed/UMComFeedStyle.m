...
- (void)resetWithFeed:(UMComFeed *)feed
{
    ...
    if (feed.text && [feed.status intValue] < FeedStatusDeleted) {
        ...
        UMComMutiText *mutitext = [UMComMutiText mutiTextWithSize:CGSizeMake(self.subViewWidth, MAXFLOAT) font:FeedFont string:feedSting lineSpace:TextViewLineSpace checkWords:feedCheckWords];
        self.feedStyleTextModel = mutitext;
        
        mutitext.textSize = CGSizeMake(mutitext.textSize.width, mutitext.textSize.height + OriginFeedHeightOffset);
        ...
        if (self.feedType == feedDetailType && [UMComTools getStringLengthWithString:feedSting] > kFeedContentLength) {
            mutitext.textSize = CGSizeMake(mutitext.textSize.width, mutitext.textSize.height + DeltaHeight);
        }
    }
    ...
    if (origin_feed) {
        ...
        UMComMutiText *originMutitext = [UMComMutiText mutiTextWithSize:CGSizeMake(self.subViewWidth-FeedAndOriginFeedDeltaWidth, MAXFLOAT) font:FeedFont string:originFeedStr lineSpace:TextViewLineSpace checkWords:originFeedCheckWords];
        self.originFeedStyleTextModel = originMutitext;
        originMutitext.textSize = CGSizeMake(originMutitext.textSize.width, originMutitext.textSize.height + OriginFeedHeightOffset);
        totalHeight += originMutitext.textSize.height + OriginFeedOriginY;
