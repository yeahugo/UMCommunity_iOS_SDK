...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.layer.borderColor = [UMComTools colorWithHexString:FontColorBlue].CGColor;
...
- (void)reloadPages
{
    ...
    for (int index = 0; index < self.pageNumArray.count; index++) {
        ...
        if (index == self.currentPage) {
            ...
            pageNumView.itemNoticeView.hidden = YES;
        ...
        if (self.indexesOfNotices.count > 0) {
            ...
            if (isShowNotice == YES) {
                pageNumView.itemNoticeView.hidden = NO;
            }else {
                pageNumView.itemNoticeView.hidden = YES;
            }
        }else{
...
- (void)drawRect:(CGRect)rect
{
    ...
    UIColor *color = [UMComTools colorWithHexString:FontColorBlue];
    ...
    for (int index = 0; index< self.totalPages;index++) {
        if (index > 0) {
            CGContextStrokeRect(context, CGRectMake((self.frame.size.width/self.totalPages)*index, 0, 0.1, rect.size.height));
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.itemLabel.font = UMComFontNotoSansLightWithSafeSize(16);
        [self addSubview:self.itemImageView];
        [self addSubview:self.itemLabel];
        self.itemNoticeView = [[UIView alloc]initWithFrame:CGRectMake(frame.size.width-noticeViewWidth*3/2, -noticeViewWidth/2, noticeViewWidth, noticeViewWidth)];
        self.itemNoticeView.backgroundColor = [UIColor redColor];
        self.itemNoticeView.layer.cornerRadius = noticeViewWidth/2;
        self.itemNoticeView.clipsToBounds = YES;
        self.itemNoticeView.hidden = YES;
        [self addSubview:self.itemNoticeView];
...
- (void)reloadItems
{
    ...
    self.itemNoticeView.frame = CGRectMake(self.frame.size.width-noticeViewWidth*3/2, -noticeViewWidth/2, noticeViewWidth, noticeViewWidth);
    [self setNeedsDisplay];
}
