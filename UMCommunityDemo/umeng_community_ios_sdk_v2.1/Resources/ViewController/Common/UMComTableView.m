...
- (void)initDataWithFrame:(CGRect)frame
{
    ...
    [self addObserver:self forKeyPath:@"tableFooterView" options:NSKeyValueObservingOptionNew context:nil];
    ...
    if ([[[UIDevice currentDevice] systemVersion]floatValue] >= 8.0) {
        self.refreshController.footView.lineSpace.hidden = YES;
    }
}

- (void)dealloc
{
    [self removeObserver:self forKeyPath:@"tableHeaderView" context:nil];
    [self removeObserver:self forKeyPath:@"tableFooterView" context:nil];
}
...
{
    if ([keyPath isEqualToString:@"tableHeaderView"]) {
    ...
    }else if ([keyPath isEqualToString:@"tableFooterView"]){
