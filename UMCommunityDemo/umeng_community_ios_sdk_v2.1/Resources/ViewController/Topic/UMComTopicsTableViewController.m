...
- (void)viewDidLoad
{
    ...
    self.tableView = [[UMComTopicsTableView alloc]initWithFrame:CGRectMake(0, -kUMComRefreshOffsetHeight, self.view.frame.size.width, self.view.frame.size.height+kUMComRefreshOffsetHeight) style:UITableViewStylePlain];
    self.tableView.clickActionDelegate = self;
    ...
    self.tableView.headView = headView;
...
{
    self.tableView.topicFecthRequest = self.topicFecthRequest;
    [self.tableView fecthTopicsData];
}
