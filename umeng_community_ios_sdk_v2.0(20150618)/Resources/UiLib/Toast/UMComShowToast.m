...
//+ (void)deletedFeedFail:(NSError *)error
//{
//    if (![self handleError:error]){
//        if (error) {
//            if (error.code == 10004) {
//                [[self class] fetchFailWithNoticeMessage:UMComLocalizedString(@"no privilege",@"请求推荐用户失败")];
//            }else{
//                [[self class] fetchFailWithNoticeMessage:UMComLocalizedString(@"Delete_Fail",@"删除失败")];
//            }
//        } else if(!error) {
//            [[self class] fetchFailWithNoticeMessage:UMComLocalizedString(@"Delete_Success",@"删除成功")];
//        }
