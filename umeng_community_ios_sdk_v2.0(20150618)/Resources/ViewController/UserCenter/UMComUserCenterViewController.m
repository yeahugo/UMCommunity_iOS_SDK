...
- (void)viewDidLoad {
    ...
    self.focus.layer.borderColor = [UIColor blackColor].CGColor;
...
- (void)deletedFeedSucceedAction
{
    if ([self.user.uid  isEqualToString:[UMComSession sharedInstance].uid]) {
        if ([UMComSession sharedInstance].loginUser) {
            ...
            self.feedNumber.text = [[UMComSession sharedInstance].loginUser.feed_count description];
        }
    }
}
...
- (void)updateUserInfo
{
    ...
    gender.frame = CGRectMake(self.userName.frame.origin.x+self.userName.frame.size.width, self.userName.frame.origin.y + 4, 15, 15);
...
- (void)updateUserProfile
{
    ...
    UMComUserProfileRequest *userProfile = [[UMComUserProfileRequest alloc] initWithUid:uid sourceUid:nil];
...
-(IBAction)onClickFeeds:(id)sender
{
    ...
    self.fansTableView.scrollsToTop = NO;
    self.followerTableView.scrollsToTop = NO;
...
-(IBAction)onClickFollowers:(id)sender
{
    ...
    self.fansTableView.scrollsToTop = NO;
    self.followerTableView.scrollsToTop = YES;
...
-(IBAction)onClickFans:(id)sender
{
    ...
    self.fansTableView.scrollsToTop = YES;
    self.followerTableView.scrollsToTop = NO;
    self.feedsTableView.scrollsToTop = NO;
