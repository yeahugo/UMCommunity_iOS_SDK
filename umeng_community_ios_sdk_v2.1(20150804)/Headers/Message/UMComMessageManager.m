...
#define BaseApiPath @"http://test.api.wsq.umeng.com:8082/v2/"
...
+ (void)handleUserInfo:(NSDictionary *)userInfo
{
    ...
    if ([userInfo valueForKey:@"umwsq"]) {
        //[userInfo valueForKey:@"umeng_comm_afteropen_controller"] && [[userInfo valueForKey:@"umeng_comm_afteropen_controller"] isEqualToString:@""]
            ...
            if ([feed_id isKindOfClass:[NSString class]]) {
                [[UMComAction action] performActionAfterLogin:feed_id viewController:presentedViewController completion:^(NSArray *data, NSError *error) {
                    if (!error) {
                        ...
                        if (extraDic.count > 0) {
                            NSData *jsonData = [NSJSONSerialization dataWithJSONObject:extraDic
                                                                               options:NSJSONWritingPrettyPrinted error:nil];
                            viewExtra = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
                        }
                        
                        
                        UMComFeedDetailViewController *feedDetailVC = [[UMComFeedDetailViewController alloc]initWithFeed:feed_id commentId:comment_id viewExtra:viewExtra];
                        if ([userInfo valueForKey:@"comment_id"]){
