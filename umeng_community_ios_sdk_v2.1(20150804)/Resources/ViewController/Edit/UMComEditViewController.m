...
- (void)viewDidLoad
{
    ...
    if (self.draftFeed) {
        ...
        self.editViewModel.topics = [NSMutableArray arrayWithArray:self.draftFeed.topicIDs];
        self.editViewModel.followers = [NSMutableArray arrayWithArray:self.draftFeed.atUserIds];
    }
    
    if (self.editViewModel.postImages) {
...
//                [self.editViewModel.topics removeObject:topic];
        ...
        for (UMComUser *follower in self.editViewModel.followers) {
            ...
            for (NSTextCheckingResult *result in matches) {
                ...
                if ([resultString isEqualToString:follower.name] || [resultString hasPrefix:@"@"]) {
                    match = YES;
                    break;
                }
            }
...
-(void)keyboardDidShow:(NSNotification*)notification
{
    ...
    self.topicsViewController.tableView.frame = self.topicsViewController.view.bounds;
}
...
- (void)dealWhenPostFeedFinish:(NSDictionary *)responseObject error:(NSError *)error
{
    if (error) {
        ...
        if ([responseObject valueForKey:@"err_code"]) {
            ...
            [self sendNotificationPostFeedResult:self.forwardFeed];
...
- (void)sendNotificationPostFeedResult:(id)object
{
  [[NSNotificationCenter defaultCenter] postNotificationName:kNotificationPostFeedResult object:object];
}
