...
@property (nonatomic, strong) NSArray *comentReplyList;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.commentReplyTableView = [[UMComSysCommentTableView alloc]initWithFrame:CGRectMake(0, self.menuControlView.frame.size.height-kUMComRefreshOffsetHeight, self.frame.size.width, self.frame.size.height-self.menuControlView.frame.size.height+kUMComRefreshOffsetHeight) style:UITableViewStylePlain];
        self.commentReplyTableView.dataFecthRequest = [[UMComUserCommentsReceivedRequest alloc]initWithUid:[UMComSession sharedInstance].uid count:BatchSize];
        [self addSubview:self.commentReplyTableView];
        self.commentReplyTableView.headView = [self headView];
        self.commentReplyTableView.footView = [self footView];
...
{
    UMComRefreshView *headView = [[UMComRefreshView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, kUMComRefreshOffsetHeight)];
    return headView;
}

- (UMComRefreshView *)footView
{
    UMComRefreshView *footView = [[UMComRefreshView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, kUMComRefreshOffsetHeight)];
    footView.isPull = NO;
    return footView;
}
...
{
    if (self.index == 0) {
        [self.commentReplyTableView refreshData:nil];
    }else{
        [self.commentTableView refreshData:nil];
    }
}
...
- (void)clikOnRightButton
{
    if (!self.commentTableView) {
        ...
        self.commentTableView.dataFecthRequest = [[UMComUserCommentsSentRequest alloc]initWithUid:[UMComSession sharedInstance].uid count:BatchSize];
        [self insertSubview:self.commentTableView belowSubview:self.menuBGView];
        self.commentTableView.headView = [self headView];
        self.commentTableView.footView = [self footView];
        [self.commentTableView fecthAllData];
    }
