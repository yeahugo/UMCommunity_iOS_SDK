...
@property (nonatomic, assign) BOOL isLoadingFinsh;
...
- (void)setHeadView:(UMComRefreshView *)headView
{
    ...
    _headView.frame = CGRectMake(0, self.headerViewHeight-headView.frame.size.height, headView.frame.size.width, headView.frame.size.height);
    [self addSubview:headView];
}

- (void)setFootView:(UMComRefreshView *)footView
{
    if (_footView != footView) {
        [_footView removeFromSuperview];
    }
    _footView = footView;
    _footView.refreshDelegate = self;
    _footView.frame = CGRectMake(0, self.frame.size.height-footView.frame.size.height, footView.frame.size.width, footView.frame.size.height);
...
{
    CGFloat itemWidth = (collectionView.frame.size.width-2*5)/4;
...

{
    if ([kind isEqualToString:UICollectionElementKindSectionFooter]) {
        ...
        UICollectionReusableView *hearderView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"header" forIndexPath:indexPath];
        return hearderView;
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.isLoadingFinsh == YES) {
        //下拉刷新
        if (offset < 0) {
            [self.headView refreshScrollViewDidScroll:scrollView];
        } //上拉加载更多
        else if (_isHasNextPage == YES){
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    if (self.isLoadingFinsh == YES) {
        ...
        }else if (self.isHasNextPage == YES){//上拉加载更多
            [self.superview bringSubviewToFront:self.footView];
