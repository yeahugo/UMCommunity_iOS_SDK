...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat likeViewOriginX = self.likeCountLabel.frame.size.width+self.likeCountLabel.frame.origin.x + 5;
        CGFloat likeUserViewWidth = frame.size.width-likeButtonOriginX-likeViewOriginX+3;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < self.itemCount-1) {
        ...
        CGPoint imageOrigin = cell.portrait.frame.origin;
        CGFloat deltal = 1.5;
        CGSize currentSize = CGSizeMake(cell.portrait.frame.size.width+deltal*2, cell.portrait.frame.size.height+deltal*2);
        cell.portrait.frame = CGRectMake(imageOrigin.x-deltal, imageOrigin.y - deltal, currentSize.width, currentSize.height);
    }
    return cell;
}
