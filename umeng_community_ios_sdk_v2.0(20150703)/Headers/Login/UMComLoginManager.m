...
+ (void)handleLoginUser:(UMComUserAccount *)userAccount response:(id)responseObject error:(NSError *)error completion:(LoadDataCompletion)completion
{
    [[UMComUpdateProfileAction action] loginSuccessPerformAction:error response:@[userAccount] viewController:nil completion:^(NSArray *data, NSError *error) {
        ...
        UMComUserTopicsRequest *userTopicController = [[UMComUserTopicsRequest alloc] initWithUid:uid count:FocusTopicNum];
        [UMComSession sharedInstance].isFocus = YES;
        [userTopicController fetchRequestFromServer:^(NSArray *data, BOOL haveNextPage, NSError *error) {
            [[UMComSession sharedInstance].focus_topics addObjectsFromArray:data];
        }];
