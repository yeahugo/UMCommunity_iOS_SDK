...
@property (nonatomic,strong) UMComAddedImageView *addedImageView;
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    self.title = @"新鲜事";
//    textFont = UMComFontNotoSansLightWithSafeSize(15);
    ...
    //添加站位语句
    placeholderLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, -4.5, self.realTextView.frame.size.width-10, 40)];
    ...
    if (self.originFeed) {
        ...
        self.atFriendButton.center = CGPointMake(self.atFriendButton.superview.frame.size.width/2, self.atFriendButton.center.y);
...
- (void)setUpAddedImageView:(NSArray *)images
{
    if(!self.addedImageView)
    {
        ...
        self.addedImageView = [[UMComAddedImageView alloc] initWithUIImages:nil screenWidth:self.view.frame.size.width];
        ...
        self.addedImageView.frame = CGRectMake(0, self.locationBackgroundView.frame.size.height, self.view.frame.size.width, 80);
...
- (void)viewsFrameChange
{
    ...
    if (self.originFeed) {
        self.realTextView.frame = CGRectMake(self.realTextView.frame.origin.x, self.realTextView.frame.origin.y, self.realTextView.frame.size.width,visibleHeight - self.forwardFeedBackground.frame.size.height - 3);
        self.forwardFeedBackground.frame = CGRectMake(self.forwardFeedBackground.frame.origin.x, self.realTextView.frame.origin.y+self.realTextView.frame.size.height, self.forwardFeedBackground.frame.size.width,self.forwardFeedBackground.frame.size.height);
    }else{
        ...
        self.realTextView.frame = CGRectMake(self.realTextView.frame.origin.x, self.realTextView.frame.origin.y, self.realTextView.frame.size.width,visibleHeight-forwordViewHeight);
        self.forwardFeedBackground.frame = CGRectMake(0, self.realTextView.frame.origin.y+self.realTextView.frame.size.height, self.view.frame.size.width,forwordViewHeight);
    }
}
...
-(void)keyboardWillShow:(NSNotification*)notification
{
   float endheight = [[[notification userInfo] objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue].size.height;
    ...
    self.editToolView.frame = CGRectMake(self.editToolView.frame.origin.x, self.view.frame.size.height - endheight - self.editToolView.frame.size.height, self.editToolView.frame.size.width, self.editToolView.frame.size.height);
    [self viewsFrameChange];
    ...
    UMLog(@"keyboardWillShow:%@",self.topicsViewController.tableView);
    UMLog(@"keyboardWillShow:%@",NSStringFromCGRect(self.topicsViewController.tableView.frame));
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if (textView == self.realTextView) {
        [self.editViewModel.editContent setString:textView.text];
        NSMutableAttributedString *mutiAttributString = [[NSMutableAttributedString alloc]initWithString:textView.text];//[[NSMutableAttributedString alloc]initWithAttributedString:textView.attributedText];
