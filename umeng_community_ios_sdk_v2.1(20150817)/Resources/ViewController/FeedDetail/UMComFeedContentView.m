...
- (void)reloadDetaiViewWithFeedStyle:(UMComFeedStyle *)feedStyle viewWidth:(CGFloat)viewWidth
{
    ...
    if (feedStyle.feedType == feedFavourateType) {
        self.collectionButton.hidden = NO;
    }
    
    if (feed.origin_feed) {
        locationName = [feed.origin_feed.location valueForKey:@"name"];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(customObj:clickOnAddCollection:)]) {
        [self.delegate customObj:self clickOnAddCollection:self.feed];
    }
}
