...
- (instancetype)initWithLocation:(CLLocation *)location title:(NSString *)title
{
    ...
    self.title = title;
    self.fetchFeedsController = [[UMComNearbyFeedsRequest alloc]initWithLocation:location count:BatchSize];
...
- (void)viewDidLoad {
    ...
    self.feedsTableView.showDistance = YES;
...
{
    self.fetchFeedsController = [[UMComNearbyFeedsRequest alloc]initWithLocation:manager.location count:BatchSize];
    [self refreshDataFromServer:nil];
}
