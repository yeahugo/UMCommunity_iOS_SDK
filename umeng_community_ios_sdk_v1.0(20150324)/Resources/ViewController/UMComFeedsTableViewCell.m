...
//#define LikeNumberRightSideWith  8
...
+(void)initialize {
    if(!Cell){
        ...
        Cell.commentTextViewWidth = Cell.commentTableView.frame.size.width - 40;
    }
}
...
-(void)reload:(UMComFeed *)feed tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    CGRect feedTextRect = [UMComMutiStyleTextView boundingRectWithSize:CGSizeMake(Cell.fakeTextView.frame.size.width, MAXFLOAT) font:feedFont string:feedString lineSpace:TextViewLineSpace];
    self.fakeTextView.frame = CGRectMake(Cell.fakeTextView.frame.origin.x, totalHeight , Cell.fakeTextView.frame.size.width, feedTextRect.size.height);
    ...
    if (self.feed.origin_feed && !self.feed.origin_feed.isFault) {
        ...
        self.fakeOriginTextView.frame = CGRectMake(Cell.fakeOriginTextView.frame.origin.x, OriginFeedOriginY, self.fakeOriginTextView.frame.size.width, originFeedRect.size.height + OriginFeedHeightOffset/2+4);
        ...
        UIImage *resizableImage = [[UIImage imageNamed:@"origin_image_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(20, 0, 0, 0)];
    ...
    if ([imagesArray count] == 0) {
        ...
        self.gridView.frame = CGRectMake(self.gridView.frame.origin.x,  totalBgHeight, Cell.gridView.frame.size.width, ceil((float)imagesArray.count/3) *  (Cell.gridView.frame.size.height+ImageSpace) + ImageSpace);
        if (self.feed.origin_feed  && !self.feed.origin_feed.isFault) {
            self.gridView.center = CGPointMake(Cell.gridView.center.x + 7, self.gridView.center.y);
        } else {
            self.gridView.center = CGPointMake(Cell.gridView.center.x, self.gridView.center.y);
        }
    ...
    self.imagesBackGroundView.frame = CGRectMake(Cell.imagesBackGroundView.frame.origin.x, totalHeight, Cell.imagesBackGroundView.frame.size.width, totalBgHeight);
    ...
    if (likeNum == 0) {
        ...
        if ((Cell.likeListTextView.frame.size.width - lastLineWith) < likeNumWith) {
            likeTextViewHeight += likeNumHeight;
        }
        self.likeNumLabel.text = [NSString stringWithFormat:@"%d",likesNum];
        self.likeListTextView.frame = CGRectMake(Cell.likeListTextView.frame.origin.x, 0, self.likeListTextView.frame.size.width, likeTextViewHeight);
        self.likeNumLabel.frame = CGRectMake(Cell.likeListTextView.frame.size.width-likeNumWith, likeTextViewHeight-likeNumHeight+1, likeNumWith, likeNumHeight);
    ...
    if (self.feed.comments.count == 0) {
        ...
        self.commentCellHeightArr = [UMComFeedsTableViewCell commentCellHeightArrWithComments:self.reloadComments withFrameWidth:Cell.commentTextViewWidth isShowAllComment:isShowAllComment feed:feed];
        [self.commentTableView reloadData];
        self.commentTableView.hidden = NO;
        self.commentTableView.frame = CGRectMake(self.commentTableView.frame.origin.x, totalHeight, Cell.commentTableView.frame.size.width, self.commentTableView.contentSize.height);
...
+ (float)getCellHeightWithFeed:(UMComFeed *)feed isShowComment:(BOOL)isShowComment
{
    float totalHeight = Cell.userNameLabel.frame.size.height + DeltaHeight;
    NSString * feedSting = @"";
    if (feed.text) {
        feedSting = feed.text;
        CGRect fitRect = [UMComMutiStyleTextView boundingRectWithSize:CGSizeMake(Cell.fakeTextView.frame.size.width, MAXFLOAT) font:UMComFontNotoSansLightWithSafeSize(15) string:feedSting lineSpace:TextViewLineSpace];
    ...
    if (origin_feed) {
        ...
        CGRect originTextRect = [UMComMutiStyleTextView boundingRectWithSize:CGSizeMake(Cell.fakeOriginTextView.frame.size.width, MAXFLOAT) font:UMComFontNotoSansLightWithSafeSize(15) string:originText lineSpace:TextViewLineSpace];
    ...
    if(feed.likes.count > 0) {
        ...
        [likeString appendString:@" "];
        NSDictionary *likeRectDict = [UMComMutiStyleTextView rectWithSize:CGSizeMake(Cell.likeListTextView.frame.size.width, MAXFLOAT) font:UMComFontNotoSansLightWithSafeSize(14) AttString:likeString lineSpace:LikeViewLineSpace];
        CGRect likeRect = CGRectFromString([likeRectDict valueForKey:@"rect"]);
        int likesNum = (int)feed.likes.count;
        CGFloat likeNumWith = 10;
        CGFloat likeNumHeight = [[likeRectDict valueForKey:@"lineHeight"] floatValue];
        ...
        if ((Cell.likeListTextView.frame.size.width - shortestWith) < likeNumWith) {
            likeTextViewHeight += likeNumHeight;
        }
    ...
    if (feed.comments.count > 0) {
        ...
        NSArray *commentCellHeightArr = [UMComFeedsTableViewCell commentCellHeightArrWithComments:reloadComments withFrameWidth:Cell.commentTextViewWidth isShowAllComment:isShowComment feed:feed];
...
-(IBAction)onClickComment:(id)sender
{
    ...
    [[UMComCommentAction action] performActionAfterLogin:self.feed.feedID viewController:self.tableView.viewController completion:^(NSArray *data, NSError *error) {
        if (!error) {
            if (!uid) {
                [[(UMComFeedTableViewController *)self.tableView.viewController resultArray] insertObject:[UMComSession sharedInstance].commentFeed atIndex:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGRect rect = [UMComMutiStyleTextView boundingRectWithSize:CGSizeMake(Cell.commentTextViewWidth,MAXFLOAT) font:font string:replayStr lineSpace:CommentViewLineSpace];
    cell.textView.frame = CGRectMake(cell.textView.frame.origin.x,0,Cell.commentTextViewWidth, rect.size.height+ComTextViewHeightOffset/2);
